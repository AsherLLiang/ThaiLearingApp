...
| 8. 配置与API | 07-Configuration-and-API-Guide.md | 常量与端点管理 |
...
| **配置API** | 集中管理常量、端点和后端配置 | 07 |

---

# 8. 配置与 API

> **详细内容**：[07-Configuration-and-API-Guide.md](./07-Configuration-and-API-Guide.md)
...
### 1. `constants.ts` (应用常量)
- **COLLECTIONS**: 数据库集合名 (`users`, `courses`)
- **API_TIMEOUT**: 请求超时设置
- **USER_ROLES**: 用户角色定义

### 2. `api.endpoints.ts` (API 端点)
- 定义所有后端接口路径
- 结合 `EXPO_PUBLIC_API_BASE_URL` 使用

### 3. `backend.config.ts` (后端配置)
- CloudBase 环境 ID 和地域配置

## 最佳实践
- **✅ 始终导入**：使用 `COLLECTIONS.USERS` 而非字符串
- **❌ 拒绝硬编码**：不要在组件中写 URL
- **❌ 拒绝敏感信息**：密钥放环境变量
